...
def handle_user_login(username, email, sub, access_token, id_token):
    ...
    if user:
        ...
        if existing_user_by_username:
            ...
            # User does not exist by Cognito ID or by username - create a new user
            try:
                ...
                flash("Failed to create user profile due to a database conflict (username might be taken). Please try again or contact support.", "error")
                return redirect(url_for('auth.login'))
            except Exception as e:
                db.session.rollback()
